...
import { CONCEPTOS_POR_TIPO, formatMovimientoQuantity, getConceptoLabel, getMovimientoQuantityTone } from "@/lib/movimientos";
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-5 animate-in fade-in duration-300">
      ...
      <div className="space-y-5">
        <section className="relative z-20 mx-auto max-w-5xl space-y-4">
          <Card className="overflow-visible rounded-md shadow-sm">
            ...
            <CardContent className="overflow-visible p-4">
              <form onSubmit={handleSubmit} className="grid gap-4 lg:grid-cols-[minmax(280px,0.95fr)_minmax(320px,1.05fr)]">
                <div className="space-y-4">
                  <section className="space-y-2">
                    ...
                    <div className="relative z-30">
                      ...
                      <Input aria-label="Buscar producto para movimiento" value={productSearch} onFocus={() => setProductListOpen(true)} onChange={(event) => { setProductSearch(event.target.value); setProductListOpen(true); }} placeholder="Nombre, variante o SKU" className="h-10 pl-9" />
                      {productListOpen && productSearch && (
                        <div className="absolute left-0 right-0 z-[80] mt-1 max-h-72 overflow-y-auto rounded-md border border-border bg-popover shadow-xl">
                  ...
                  <section className="space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-2" aria-label="Tipo de movimiento">
                      {(Object.keys(TYPE_META) as TipoMovimientoStock[]).map((type) => {
                        const meta = TYPE_META[type];
                        const Icon = meta.icon;
                        return <button key={type} type="button" onPointerMove={updateLiquidGlassPointer} onClick={() => selectType(type)} className={`liquid-choice flex h-12 flex-col items-center justify-center rounded-md border text-xs font-bold transition-colors ${form.tipoMovimiento === type ? `${meta.active} ring-1 ring-current/15` : "border-border text-muted-foreground"}`}><Icon className="mb-0.5 size-3.5" />{meta.label}</button>;
                      })}
                    </div>
                  ...
                  <section className="space-y-2">
                    ...
                    <div className={`grid gap-3 ${isCommercial ? "sm:grid-cols-2" : "grid-cols-1"}`}>
                    ...
                    <div className={`rounded-md border p-2.5 ${resultingStock !== null && resultingStock < 0 ? "border-rose-500/40 bg-rose-500/10" : "border-border bg-muted/30"}`}>
                  ...
                  {isExchange && <div className="flex gap-2 rounded-md border border-blue-500/30 bg-blue-500/10 p-2.5 text-xs text-blue-800"><ArrowRightLeft className="mt-0.5 size-4 shrink-0" /><p>Usa el mismo código en la entrada del producto recibido y en la salida del producto entregado.</p></div>}
                  {form.concepto === "CAMBIO_GARANTIA" && <div className="flex gap-2 rounded-md border border-amber-500/30 bg-amber-500/10 p-2.5 text-xs text-amber-800"><TriangleAlert className="mt-0.5 size-4 shrink-0" /><p>El artículo defectuoso no vuelve al stock vendible. Esta salida registra el costo del reemplazo.</p></div>}
                </div>
                ...
                <div className="space-y-4">
                  <section className="space-y-2">
                    ...
                    <label className="block space-y-1.5 text-sm font-semibold"><span>Notas</span><Textarea aria-label="Motivo del movimiento" value={form.motivo} onChange={(event) => setForm({ ...form, motivo: event.target.value })} placeholder="Estado, proveedor, cliente u observación" className="min-h-16" /></label>
                  </section>
                  ...
                  <div className="flex justify-end">
                    <Button disabled={saving || !selectedInventory || resultingStock === null || resultingStock < 0} className="h-10 w-full px-6 sm:w-auto" type="submit">{saving ? <RefreshCw className="mr-2 size-4 animate-spin" /> : <ClipboardList className="mr-2 size-4" />}{saving ? "Guardando..." : `Registrar ${TYPE_META[form.tipoMovimiento].label.toLowerCase()}`}</Button>
                  </div>
                </div>
              </form>
            </CardContent>
        ...
        <section className="relative z-0 min-w-0 space-y-3">
                ...
                <Table>
                  ...
                  <TableBody>
                    {movimientos.length === 0 ? <TableRow><TableCell colSpan={7} className="h-48 text-center text-muted-foreground">No hay movimientos para los filtros seleccionados.</TableCell></TableRow> : movimientos.map((movement) => {
                      ...
                      const quantityTone = getMovimientoQuantityTone(movement);
